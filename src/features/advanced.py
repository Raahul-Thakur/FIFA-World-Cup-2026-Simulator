...
def compute_elo_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Use whichever column name the features.csv has (elo.py uses home_elo_before,
    # but builder.py renames it to home_elo in the saved features.csv)
    home_elo_col = "home_elo_before" if "home_elo_before" in df.columns else "home_elo"
    away_elo_col = "away_elo_before" if "away_elo_before" in df.columns else "away_elo"

    # Absolute team level (average Elo) — top teams convert narrow margins into wins
    df["avg_elo"] = (df[home_elo_col] + df[away_elo_col]) / 2

    # Elo momentum: current Elo minus value 10 matches ago (positive = improving form)
    home_elo_trend = df.groupby("home_team")[home_elo_col].transform(
        lambda s: s.diff(10).fillna(0)
    )
    away_elo_trend = df.groupby("away_team")[away_elo_col].transform(
        lambda s: s.diff(10).fillna(0)
    )
